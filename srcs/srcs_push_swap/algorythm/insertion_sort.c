...
/*   Updated: 2021/04/28 14:59:46 by lvirgini         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	optimize_insertion_sort(t_stack *a, t_stack *b)
{
	...
	op = optimize_push_to_pop(a, b, b->size);
	if (op.total > 3)
	...
	{
		if (op.total == op.nb_rotate && op.nb_rotate != 0)
		{
			print_and_do(a, b, RR);
			op.nb_rotate --;
		}
	...
	print_and_do_multi(a, b, op.b_type_rotate == ROTATE ? RB : RRB, op.b_rotate);
	print_and_do(a, b, PA);
}
...
**		- get best push a (first, second or last)
**		- push ending b on order to a.
*/
